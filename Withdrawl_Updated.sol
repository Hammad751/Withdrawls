...
contract Withdrawl is SignVerify, Ownable, Pausable, ReentrancyGuard{
    ...
    uint256 public Amount = 10500000000000000000000;
    uint256 public MaticAmount = 400000000000000000;
    ...
    function userWithdraw(uint256 amount, uint256 MATICamount, uint256 _nonce, bytes memory signature)
    external
    nonReentrant
    whenNotPaused
    {
        ...
        require(MATICamount <= MaticAmount, " Daily Limit Exceeds");
        internalWithdraw(amount, MATICamount, _nonce, signature);
    }

    function internalWithdraw(uint256 amount, uint256 MATICamount, uint256 _nonce, bytes memory signature)
    internal
    {
    ...
    function setTokenAmount(uint256 _amount)
    public
    onlyOwner
    {   Amount = _amount;   }

    function setMATICAmount(uint256 _MATICamount)
    public
    onlyOwner
